        ...
        public:
            ...
            */
            virtual void async_recv() {
                ...
                if (WSARecv(get_socket_fd(), &wsabuf, 1, &bytes, &flags, (LPOVERLAPPED) &ctx, nullptr) ==
                    SOCKET_ERROR) {
           ...
           *@return void
            ...
            */
            virtual void async_sendv(const iovec_t *iov, size_t count, bool flush = true) {
                ...
                if (WSASend(get_socket_fd(), &wsabuf, 1, &bytes, 0, (LPOVERLAPPED) &get_send_context(), nullptr) ==
                    SOCKET_ERROR) {
                    if (WSAGetLastError() != WSA_IO_PENDING) {
...
//                         close();
            ...
            void dump_error(const char *title) {
                std::cout << title << " fd:" << get_socket_fd() << " remote:" << get_remote_addr().get_address_string()
                          << " local:" << get_local_addr().get_address_string() << " error:" << socket::get_error()
                          << " " << socket::get_error_str(socket::get_error()) << std::endl;
            }
        ...
        private:
            ...
            bool async_accept() {
                ...
                if (get_acceptex_func()(get_socket_fd(), get_new_conn_fd(), _addr_buf, 0, address_size, address_size,
                                        &_bytes_len,
            ...
            void on_accept(bool status) {
                ...
                if (status) {
                    if (setsockopt(get_new_conn_fd(), SOL_SOCKET, SO_UPDATE_ACCEPT_CONTEXT, (char *) &get_socket_fd(),
                                   sizeof(get_socket_fd())) != 0) {
                        socket::close(get_new_conn_fd());
        ...
        public:
            ...
            */
            ...
            bool add_connector(const string &name, const string &conn_addr, connect_callback_t cb,
                               const string bind_addr = "0.0.0.0:0", bool reconnect = true,
                               uint32 reconnect_delay = 5000) {
                if (_iocp == nullptr)
                    return false;
